...
> 状态：**流水线已实现，等待凭据启用**。
>
> **已落地（自动化，无需再改）**：`scripts/build_macos_app.sh` 的 `notarize_and_staple` 与发布工作流 `build-macos-packages.yml` 的按需签名/公证已实现——**只有配置了 `MACOS_SIGNING_IDENTITY` 等 secret 才会签名+公证，否则严格回退到现有 ad-hoc 构建**。因此你现在只需完成「阶段 A 申请 Apple 凭据 → 阶段 B 配置 6 个 GitHub Secret → 阶段 D 打一次 `-rc` 测试发布验证」，无需等我再改这两处代码（下文步骤 7–8 已完成，保留作对照）。
>
> **尚未启用（属独立的 App 代码改动，见步骤 9–10）**：自动更新锚定发布者 Team ID、收紧密钥串 ACL——这两项需在首个 Developer ID 正式版落地后再合并，我会在你验证通过后单独提交。
>
